...
- In the Northwind database, what is the type of relationship between the
  ...
  * The relationship between Employee and Territory is one to many. Employee.Id
    is the primary key of the Employee table, and each employee id is matched with
    at least one (but potentially up to 10) territory ids. The territory ids are 
    the primary key in the Territory table (Territory.Id), and employee ids are 
    matched up with territory ids in the EmployeeTerritory table, where both 
    function as foreign keys. Employees has a one to many relationship with
    EmployeeTerritory, while Territory has a one to one relationship with
    Territory.
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  * A document store like MongoDB is useful when when you don't 
    require strong ACID guarantees. While it won't cut it for banking 
    transactions, it could be useful for something like a content management
    system for blog posts. It is also useful when you want to do fast 
    prototyping that doesn't require a lot of work defining a schema in a 
    relational database. Finally, NoSQL documnent stores can be distributed
    elastically and scale on demand.
- What is "NewSQL", and what is it trying to achieve?
  * NewSQL is an attempt to combine the best aspects of SQL based relational
    database management systems (RDBMS) and NOSQL document stores. It is a 
    class of RDBMS that claims to provide the scalability that NoSQL offers, 
    while still promising strong ACID (Atomicity, Consistency, Isolation, Durability)
    guarantees. If it can deliver on these promises, that would mean that it would 
    offer enterprises that handle large amounts of sensitive data an alternative 
    to purchasing more powerful computers or developing custom middleware.
