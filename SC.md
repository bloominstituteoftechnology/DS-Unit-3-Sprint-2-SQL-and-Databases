...
- In the Northwind database, what is the type of relationship between the
  ...
  -- The Employee and Territory tables share a many-to-many relationship. You can figure this out because they don't share keys between each other but instead have a separate table which contains foreign keys for the two tables. This allows a more complicated relationship than just each employee having a single territory assigned to them.
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  -- A Document store is appropriate when you're using data without a formalized structure, for instance if you're scraping web pages the end result will often be a very different structure for each page so trying to jam it into a SQL Schema would be impossible. By using JSON (or BSON) we can achieve a formalized structure which is easy to decode but also allows us to put in a wide variety of data without a complicated schema.
- What is "NewSQL", and what is it trying to achieve?
  -- NewSQL attempts to blend the best qualities of SQL databases and noSQL Document Stores. They retain the ACID guarantee from SQL databases while adding in sharding and other scalability improvements that noSQL is known for. A downside does seem to be that they are optimized for transaction heavy workflows and perhaps might not be ideal for situations where large amounts of joins and other computation needs to be done by the database engine.
