...
- In the Northwind database, what is the type of relationship between the
  ...
  1 to many, ach Employee territory has exactly one employee to that territory. 

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  When you need to store data but the data you need to store is constantly changing.
  Like each thing you need to store is slightly different. Document storage is great
  because you don't need to create the schema for each document like in a relational DB. 
  When structure is crucial like when keeping track of supplies, you want to see who 
  ordered what and the information on that order. Having a schema is very useful in 
  this case.

- What is "NewSQL", and what is it trying to achieve?
NewSQL is trying be scalable like noSQL while still upholding the ACID properties of 
SQL. NewSQL is trying to solve the scalability issue with SQL, this comes at a cost though.
