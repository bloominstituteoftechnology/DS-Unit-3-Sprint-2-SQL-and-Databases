...
- In the Northwind database, what is the type of relationship between the
  ...
  Employees and Territories are connected through a table called EmployeeTerriritories. Employees has a one to many relation with EmployeeTerriritories as each employee has a unique ID associated with them but can have multiple terrorites associated with each employee. The territories table has a one to one relationship with EmployeeTerritories as there is only one territoryId per territory. The relationship between Employee and Territory is one to many as each employee is unique but they may have one or more than one territory associated with them. Summing it up, terroitories may have many employees associated with one territory.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

A Nosql database is great in a lot of areas, a few being:

1. You need to store large amounts of data that is unstructured (think text)
2. If you need to scale rapidly
3. Less db planning and more development.

A Nosql db can fall short as well, a few being:

1. If you need to be ACID compliant
2. Your data is structured and should be stored that way
3. You don't expect a lot of changes in db structure moving forward

- What is "NewSQL", and what is it trying to achieve?

NewSQL is tring to be the best of both worlds bridging the gap between NoSQL and SQL. It brings ACID compliance to a NoSQL database so data sensitive companies can get the benefits of the unstructured ways of NoSQL. For companies considering a relational db but need the scalablity of the unstructure NoSQL, then the NewSQL choice may be the right one. 
