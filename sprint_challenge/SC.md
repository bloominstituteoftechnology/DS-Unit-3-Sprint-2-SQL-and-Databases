...
- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables?
    ...
    - The relationship between employees and territories is many to many. You can have many employees who each
    can have many territories. Just look at Robert King up there. He has ten categories and is just one of many
    employees.
...
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
    ...
    - Document storage like MongoDB is appropriate for large amounts of unstructured data, like a shopping cart or 
    social media metadata, where a single user generates and uses data unrelated to other users. In this way data
    can be distributed without regard to a general schema.


- What is "NewSQL", and what is it trying to achieve?

    - NewSQL is an attempt to bring together the best of relational database structures, with their ACID guarantees, and 
    the speed and ease of distribution inherent in document storage systems. Of consistency, availability, and 
    partition tolerance, we must choose two to be true at any one time. Most attempts seem to be choosing availability
    and partition tolerance and accepting a BASE or Basically Available Soft state Eventually available. Which means that
    a user may not get the newest data every time, but that updates will eventually propagate throughout the system and
    reach every user.
