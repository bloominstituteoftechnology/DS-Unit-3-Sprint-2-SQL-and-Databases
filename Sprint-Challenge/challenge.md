...
- In the Northwind database, what is the type of relationship between the
  ...
  The `Employee` and `Territory` tables have a many-to-many relationship. Each
  employee may participate in multiply territories, and each territory can have
  multiple employees.
  
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  
  MongoDB is appropriate for prototyping an app where we don't yet know what we'll be storing,
  so the schema of the data isn't yet well defined. It's often enough for quick and dirty
  application development and is distributed in the cloud which allows faster scaling.
  
  MongoDB is not so ACID compliable (latest versions are better) and so isn't used to 
  store sensitive information. It is not used when other traditional SQL infrastructures
  are already in place. 
  
- What is "NewSQL", and what is it trying to achieve?

  Approaches that aim to achieve scalability and performance of non-relational databases
  with the ACID compliance of relational Databases.
