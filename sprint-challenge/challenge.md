...
- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables?
    'Employee' has a one-to-many relationship to 'Territory'. Entries in 
    'Employee' may map to multiple entries in 'Territory', because each
    employee may be responsible for multiple territories

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
    Document stores are appropriate when a loose schema or no schema isn't a
    concern, and when availability and performance is favored. Maybe you're 
    in development or rolling out new functionality for your app, and your 
    schema would need to be updated over and over.
    Document stores are inappropriate when the guarantees provided by schemas
    and SQL-style databases are warranted - atomiticity, consistency, isolation
    and durability.

- What is "NewSQL", and what is it trying to achieve?
    NewSQL is a sort of third generation of DBMS - if NoSQL was a response to
    SQL's rigidity, favoring scaling and availability but compromising on 
    guarantees, NewSQL techniques tend to try to find a middle ground, 
    using relational models but aiming for better scalability, generally
    by using partitioning, while also satisfying ACID.
