...
- In the Northwind database, what is the type of relationship between the
  ...
  > This is a `many to one` relationship. Each Territory has 1 employee assigned
    to it, but an employee can have multiple territories. This gets weird with 
    the number of distinct territories in the Territory Table (52) and Employee
    Territory Table (49), indicating that 3 territories are unaccounted for. If employees
    were allowed to overlap and be assigned to the same territory, then it would be 
    many to many. However, if each employee were assigned a single territory, and each 
    territory has a unique employee then the relationship would be one to one. 
    
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  > MongoDB is a silly sounding unstructured DataBase service. It is intended
    (as per it's namesake) to host 'Big Data'. Therefore it is appropriate when
    a company has an enormous amount of data the needs storage. It splits the data 
    on top multiple shards, and can still be used for data aggregation; primarily 
    through map-reduce. Although you can show relational information, it is more 
    difficult, so it should primarily be used if a company has and **needs** Big Data. 
    In situations where a company does not have a lot of data, it is much easier to 
    utilize a traditional structured database.  
  >
  > It is important to note that MongoDB like some other document stores 
    [does support](https://www.mongodb.com/faq#acid-transactions) ACID transactions. 
    
- What is "NewSQL", and what is it trying to achieve?
  > NewSQL is a type of relational database that is trying to achieve the 
    scalability of NoSQL while maintaining ACID properties. Many databases that handle 
    ACID reliant data (like a bank) are too large for conventional relational databases,
    but have requirements that are not practical for NoSQL systems. It attempts to bring
    a high degree of interactivity and reliability to highly scaled databases that primarily
    operate on SQL.  
