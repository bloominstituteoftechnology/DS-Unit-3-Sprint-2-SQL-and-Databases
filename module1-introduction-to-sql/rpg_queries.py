...
q = """SELECT COUNT(*) FROM django_content_type AS d
...
for m in c:
    print('\t', 'mage:necromancer' if m[0] == 'necromancer' else m[0], end='')
...
print(f"How many of the Items are weapons?  {c[0]} How many are not? {cn[0]})")
...
print(
    f" On average, how many Items does each Character have?  \
    {conc.execute(q).fetchone()[0]}")
