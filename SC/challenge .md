...
The `Employee` and `Territory` tables have a many-to-many relationship. An employee can have multiple territories to cover, and a territory can have multiple employees assigned to it. The data is connected with an intermediary table titled `EmployeeTerritory` that facilitates queries between the the `Employee` and `Territory` tables.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

A document store can be appropriate is a lot of documents need to be stored, but each individual document is not critically important. If occasionally a document is lost or corrupted, that is worth the price of scalability and convenience. Collecting all the cat pictures on the Internet has been given as an example. It might also make sense if the project is a prototype where speed and flexibility are more important than ACID compliance. In this case, it is probably good practice to rewrite the project using a relational database before putting it into production.

A situation where it is not appropriate would be one where data integrity is paramount importance. Banks should not use a document store for their financial records.

- What is "NewSQL", and what is it trying to achieve?

NewSQL is an attempt to have the best of both worlds. Ideally, it would provide the guarantees of an ACID compliant database along with the easy scalability of NoSQL implementations. This would allow adding extra machines as a database grows in size, rather than having to migrate to a single new machine with more memory and storage.
