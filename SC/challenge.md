...
It is a one to many relationship. Each Employee record is can be tied to multiple
EmployeeTerritory record that has a one-to-one mapping to Territory records. 


- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

NoSQL databases is appropriate for very large dataset that's document-based, 
key-value pairs, graph dB, or wide-column stores. It is horizontally scalable. 
NoSQL is not suitable for making complex queries. It is not optimal for storing 
table based data with predefined schema. NoSQL does not guarantee ACID and 
implements eventual consistency.

- What is "NewSQL", and what is it trying to achieve?

NewSQL is a new class of RMDBS that tries to combine the scalability of NoSQL while
maintaining ACID of traditional RMDBS. It favors Consistency over Availability in 
the the Consistency, Availability, and partition Tolerance Theorem. It also heavily
utilizes main memory (RAM) as storage to provide the speed of OnLine Transaction
Processing style of NoSQL.
