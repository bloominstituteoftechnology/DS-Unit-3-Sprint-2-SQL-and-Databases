...
- In the Northwind database, what is the type of relationship between the
  `Employee` and `Territory` tables?
    ...
    The `Employee` and `Territory` tables stand in a many-to-many
    relationship.  They are joined by the associative table
    `EmployeeTerritory`.  The primary key for `Employee`, 'Id', is found in 
    `EmployeeTerritory` as 'EmployeeID'.  The primary key for `Territory` is
    found in `EmployeeTerritory` as 'TerritoryID'.  The primary key for
    `EmployeeTerritory` is an 'Id' for each 'EmployeeID'/'TerritoryID' pair.
    The same 'EmployeeID' can be paired with multiple 'TerritoryID's, and the
    same 'TerritoryID' can be (though it seems, in fact, not to be) paired
    with multiple 'EmployeeID's.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

    When you are prototyping something and constantly revising, it might make
    sense to use a document store, because document-oriented databases don't
    require you to determine a schema at the outset and stick to it.  This 
    allows for more flexibility.  Document-oriented databases are also easier
    to scale.  When you need reliable access to well-defined data, however,
    a document store is not appropriate.  Banks should stick to relational
    databases with a predefined schema; fixed structure is more important 
    than flexibility here.

- What is "NewSQL", and what is it trying to achieve?

    NewSQL is the name given to a class of relational database management
    systems that try to combine the easy scalability of NoSQL approaches
    with the ACID guarantees of Atomicity, Consistency, Isolation, and
    Durability provided by relational databases.  SQL is the primary 
    interface for NewSQL systems.
