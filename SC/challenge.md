...
- In the Northwind database, what is the type of relationship between the
  ...
  - The `Employee` table has a one-to-many relationship with `Territory`, linked
    on EmployeeID. There's one unique ID for each employee in the `Employee` 
    table, but because each employee is responsible for more than one territory,
    he or she may appera multiple times in the `Territory` table. Additionally, 
    the employee ID is the primary key of the `Employee` table, which means that
    each value can only appear once. But because employee ID is a foreign key in 
    the `Territory` table, each instance can appear more than once.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  - A document store woule be appropriate when solving a problem relationally 
    is just not a suitable task. For example, Google has petabytes of data stored 
    in big data centers, and they frequently want to get as much of it as possible
    for their clients, which requires a more flexible schema, and constant scaling.
    Relational databases would be more appropriate in a situation where the natural
    relationships between entities is important, and losing data would be terrible
    (e.g. a bank).

- What is "NewSQL", and what is it trying to achieve?
  - NewSQL tries to be the best of both the NoSQL and RDBMS worlds. It aims to 
    combine the scalability of NoSQL systems while maintaining the ACID guarantees
    of RDBMS. NewSQL also offers variety in schema management, which means it can 
    be adopted for many different use-cases (something NoSQL and RDBMS have trouble
    doing - they're set in their ways, and are better suited for certain industries 
    over others). 
