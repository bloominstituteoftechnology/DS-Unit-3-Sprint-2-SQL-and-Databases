...
**`Employee` to `Territory` is a one to many relationship facilitated by the intermediate table EmployeeTerritory. From a business perspective this makes sense, there should only be one employee to a territory otherwise the employees are canabalizing each others prospects but there is no harm in multiple territories per employee. We can confirm this with SQL queries as well:**
```
SELECT COUNT(EmployeeTerritory.EmployeeID)
FROM EmployeeTerritory
GROUP BY EmployeeTerritory.TerritoryID;
```
**This gives us a count of 1 employee per terrritory. When we do the reverse below, we get multiple territories per employee.**
```
SELECT COUNT(EmployeeTerritory.TerritoryID)
FROM EmployeeTerritory
GROUP BY EmployeeTerritory.EmployeeID;
```
**While both these SQL queries are performed on `EmployeeTerritory` the relationship still holds true for `Employee` and `Territory` tables since they simply provide the supporting detail for the unique keys which are paired in `EmployeeTerritory`**

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

**A document store database is appropriate for situations where you are dealing with large amounts of unstructured data and/or don't have the luxury of creating a schema for a relational database. Thanks to the flexibility of document store systems, they scale very easily and allow for rapid development. The trade off versus a relational database is you can't ensure ACID compliance which may be an issue if you have critical information in your database such as bank records.**

- What is "NewSQL", and what is it trying to achieve?

**NewSQL is a type of relational database that attempts to blend the scalability of document store databases with the ACID compliance of a traditional system.**
