...
- In the Northwind database, what is the type of relationship between the
  ...
  It is a many to one relationship. Each employee has one territory. Each territory can have many employees. If each employee could only have one territory, it would be a one to one relationship.
  
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  
  MongoDB is built for the cloud and has a high insert rate. It is considered to be a good match for blogs, social networking sites, and e-commerce. Since MongoDB requires lots of memory, it should probably not be used for small, locally hosted projects.
- What is "NewSQL", and what is it trying to achieve? 

NewSQL is a kind of relational database management system. It's goal is to provide, simultaneously, the relational data model and ACID (Atomicity, Consistency, Isolation, and Durability) properties in addition to this. NoSQL, on the other hand, does not conform to ACID.
