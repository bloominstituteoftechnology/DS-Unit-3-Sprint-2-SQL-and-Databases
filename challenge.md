...
In this database the relationship between `Employee` and `Territory` is a many-to-many relationship.
Its many-to-many because a third junction table is required to map the primary keys from one table to the other.
  
  
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

I think a general case for using a document database might be when your data isn't fully cleaned.
In that state there is less structure and the features might have mismatched types or 
different features altogether. MongoDB is more forgiving here because as long as the data
obeys the requirements of Python dicts, then it can be stored. An example of when MongoDB may not 
be a good choice is, well, the opposite: when the data is vey structured, or when structure is 
a necessity. In this case a well-defined schema would be very important. A related point for why not 
to potentially choose MongoDB: there is no ACID guarantee.

- What is "NewSQL", and what is it trying to achieve?

NewSQL, I would say, is less a term for a product and more a term for a kind of database technology/research.
NewSQL aims to scale like NoSQL (which lack ACID guarantees) and offer ACID compliance like traditional SQL.
NewSQL is typically meant for use cases where there are many small lock-free transactions to the database.
