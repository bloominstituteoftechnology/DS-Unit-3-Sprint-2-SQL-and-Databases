...
The relationship between Employee and Territory is "one-to-many". One can see this in 
the EmployeeTerritory Table where a single EmployeeID handles multiple TerritoryIds. This 
makes sense as one assumes that each employee will have at least one distinct territory 
to service. 
...
MongoDB is an example of NoSQL. It stores data using a "document" framework (i.e. 
each observation has its own fields with data). Therefore it is useful when the data 
do not conform to a given schema. As a result, it has better scalability for "Big Data". 
It would not be appropriate for data that are already highly structured as SQL provides 
better analytical methods and reporting tools.



- (*Stretch*) What is "NewSQL", and what is it trying to achieve?

NewSQL is a RDBMS that is attempting to fix the issues of lack of scalability and 
flexibility in SQL that have caused many to flee to NoSQL.
