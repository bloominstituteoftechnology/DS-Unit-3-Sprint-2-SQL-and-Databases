...
- In the Northwind database, what is the type of relationship between the
  ...
  The two tables have a one-to-one relationship because a single record (column) exists in column. For example, two tables might have the same column 'EmployeeId' which relate to each other and can be used in joins.
- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?
  A document store (document-oriented database) is useful for unstructured data,
  it's fast to use to prototype things (such as in a startup). You don't need to build a complex schema. In the long run if you want something that gives the
  ACID guarantees it may not be as useful (bank data).
- What is "NewSQL", and what is it trying to achieve? NewSQL attempts to
replicate the scalability of NoSQL while providing the ACID consistency of SQL.
Of the CAP theorem, NewSQL is both consistent and partition-tolerant. It
emphasizes consistency over availability, meaning that it will return the most
recent query or an error, rather than returning an output regardless of error.
