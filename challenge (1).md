...
- In the Northwind database, what is the type of relationship between the
  ...
  There is a many to many relationship between the Employee and Territory tables. Employees can have multiple territories and a territory can have more than a single employee. The Territory table can be grouped with the Employee table through the columns Territory.TerritoryDescription and Employee.City. These are already linked in a table  called EmployeeTerritory, where the primary keys for the Employee and Territory tables are used as foreign keys.

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

  MongoDB is useful when there is a very large sum of data that is unstructured and does not need to be relationally linked to other data in the database (like sql does). Any company that is storing extremely large amounts of data and does not use a database administrator should use MongoDB. In situations where transactions need to be validated through ACID properties, MongoDB would be ineffective, like for storing transactions in a bank database. The data in MongoDB is stored as a collection of key value pairs(usually as JSON files), while SQL stores it as tables and rows.

- What is "NewSQL", and what is it trying to achieve?

NewSQL is a combination of SQL and NoSQL. It validates the ACID properties as well as having more scalability than regular SQL.
